...
Napi::Object ToNapiUser(Napi::Env env, const UserRecord &user)
{
    ...
    // --- access flags
    obj.Set("enable", Napi::Number::New(env, user.enable));
    obj.Set("enableChangePassword", Napi::Number::New(env, user.enable_change_password));
    obj.Set("enableReadOnly", Napi::Number::New(env, user.enable_read_only));
    obj.Set("enableOtp", Napi::Number::New(env, user.enable_otp));
    ...
    // --- dates
    obj.Set("regdate", Napi::Number::New(env, user.regdate));
    obj.Set("lastdate", Napi::Number::New(env, user.lastdate));
    obj.Set("timestamp", Napi::Number::New(env, user.timestamp));
